...
# line search with 'Guaranteed Sufficient Decrease'
#   
# Source:
...
#     phi_prime -- the series of gradients generated along the line. phi_prime(i) = phi_prime(alpha_t(i)) 
#
#     phi -- the series of objective function values generated along the line. phi(i) = phi(alpha_t(i)) 
#
#     alpha_t -- the series of trial step lengths generated. first entry is always 0.0
#
# Data about I, the interval, at each iter
#     alpha_l -- lower interval bound INDEX. The alpha value for the lower bound is thus alpha_t(alpha_l(i))
#                the phi value for the lower bound index is phi(alpha_l(i)) ect
#
#     alpha_u -- upper interval bound INDEX. entry is -1 until alpha_u is updated from its initial value of 
#                +infinity to some value between alpha_min and alpha_max
#
...
#     psi -- the value of the auxilary function at points along the line. psi(i) = psi(alpha_t(i))                                        
#
#     psi_prime -- the gradient of the auxilary function at points along the line psi_prime(i) = psi_prime(alpha_t(i))                                        
#
# Parameters 
#     alpha_init -- initial guess for line search. should be 1.0 for quasi-newton alpha_min<alpha_init<alpha_max
#
#     alpha_min -- the minimum alpha that we will allow to be generated. 0<= alpha_min.
...
#
#     alpha_max -- the maximum alpha that we will allow to be generated alpha_max will be determined in the 
#                  constructor based on the type of problem. For unconstrained optimization problems that are 
#                  bounded from below, this is just a large default value. 
#                  TODO:                       
...
#      
#     delta_min -- factor forcing a walk toward alpha_min until alpha_l is updated
...
#
#     delta_I -- if the length of the interval:
...
#      
# Internal variables 
#     iteration -- internal variable for iteration-dependent conditions as well as to index alpha, phi, and phi_prime
#     
#     mode -- we begin in GSD_MODE_PSI and use psi information to update
...
#             NB: 
#                 it is impossible to enter GSD_MODE_PHI without updating
...
class line_search:
    ...
    def init_ls_params(self, param_pref):
        if (param_pref == 0): #quasi newton
            ...
            # these should probably be tweaked, but SD is really just for testing
            ...
            raise ValueError("param pref not recognized")
        self.check_parameters()
    ...
    def check_parameters(self):
        if not ((0.0 < self.mu) and (self.mu < self.eta) and (self.eta < 1.0)):
            print "mu = ",self.mu
            print "eta = ",self.eta
            raise ValueError("must have 0 < mu < eta < 1")
        if not ((self.alpha_min >= 0.0) and (self.alpha_init > self.alpha_min) and (self.alpha_init < self.alpha_max)): 
            print "alpha_min  = ", self.alpha_min  
            print "alpha_init = ", self.alpha_init 
            print "alpha_max  = ", self.alpha_max  
            raise ValueError("must have 0 <= alpha_min < alpha_init < alpha_max")
        if not (self.delta_max > 1.0):
            raise ValueError("delta_max must be greater than 1")
        if not (self.delta_min < 1.0): 
            raise ValueError("delta_min must be less than 1")
        if not (self.delta_I < 1.0):   
            raise ValueError("delta_I must be less than 1")
    ...
    # Input:
    #     cur_phi -- the current objective function value.
    #     cur_phi_prime -- the dot of the current objective function gradient
    ...
    def next_step(self,cur_phi,cur_phi_prime):
        ...
        if (self.iteration == 0):
            ...
            if (self.mode == self.GSD_MODE_PSI):
                ...
                U2 = (self.psi[self.iteration] <= self.psi[self.alpha_l[self.iteration]]) and \
                     (self.psi_prime[self.iteration]*(self.alpha_t[self.alpha_l[self.iteration]] - self.alpha_t[self.iteration]) > 0.0)
                ...
                if ((self.iteration-ago) >= 0):
                    ...
                    if not ((self.alpha_u[self.iteration]-ago) == -1):
                        ...
                        if (abs(self.alpha_t[next_alpha_l] - self.alpha_t[next_alpha_u]) > \
                           self.delta_I*abs(self.alpha_t[self.alpha_l[self.iteration-ago]] - self.alpha_t[self.alpha_u[self.iteration-ago]])):
                            if (self.iprint>0): 
                            ...
                            if (self.iprint > 0): 
                                print "DEBUG: sufficient Interval decrease satisfied. old = "
                                str(abs(self.alpha_t[self.alpha_l[self.iteration-ago]] - self.alpha_t[self.alpha_u[self.iteration-ago]]))
                                " new = "
                                str(abs(self.alpha_t[next_alpha_l] - self.alpha_t[next_alpha_u]))
                ...
                if not (have_next_alpha):
                    ...
                    if (self.alpha_u[self.iteration] == -1):
                        # default doesn't matter because we won't use it
                        a_u = 77.7
                        f_u = 77.7
                        g_u = 77.7
                    else:
                        a_u = self.alpha_t[self.alpha_u[self.iteration]]
                        f_u = self.psi[self.alpha_u[self.iteration]]
                        g_u = self.psi_prime[self.alpha_u[self.iteration]]
                ...
                if (self.phi[self.iteration] > self.phi[self.alpha_l[self.iteration]]):
                    ...
                    next_alpha_u = self.iteration 
                #b (analog of U2)
                elif ((self.phi[self.iteration] <= self.phi[self.alpha_l[self.iteration]]) and \
                     (self.phi_prime[self.iteration]*(self.alpha_t[self.alpha_l[self.iteration]] - self.alpha_t[self.iteration]) > 0.0)):
                    next_alpha_l = self.iteration;
                    next_alpha_u = self.alpha_u[self.iteration]
                #c (analog of U3)
                else:
                ...
                if not (have_next_alpha):
                    ...
                    if (self.alpha_u[self.iteration] == -1):
                        ...
                        a_u = self.alpha_t[self.alpha_u[self.iteration]]
                        f_u = self.phi[self.alpha_u[self.iteration]] 
                        g_u = self.phi_prime[self.alpha_u[self.iteration]]
                    next_alpha = self.interpolate(a_l,a_t,a_u,f_l,f_t,f_u,g_l,g_t,g_u)
                    have_next_alpha = True
            else:
                raise RuntimeError("something is wrong with ls internal variables")
        
        # DEBUG - TODO remove eventually
        if ((next_alpha < (self.alpha_min - 1.0E-8)) or (next_alpha > (self.alpha_max + 1.0E-8))):
            print "ERROR: alpha_min = "+str(self.alpha_min)+"  alpha_max = "+str(self.alpha_max)+"  next_alpha = "+str(next_alpha)
        ...
        if not (self.alpha_u[self.iteration]==-1):
            ...
            if ((next_alpha < (min(self.alpha_t[next_alpha_u],self.alpha_t[next_alpha_l]) - 1.0E-8)) or (next_alpha > (max(self.alpha_t[next_alpha_u],self.alpha_t[next_alpha_l]) + 1.0E-8))):
                print "ERROR: next_alpha_l = "+str(self.alpha_t[next_alpha_l])+" next_alpha = "+str(next_alpha)+" next_alpha_u = "+str(self.alpha_t[next_alpha_u])
    ...
    def interpolate(self, a_l, a_t, a_u, f_l, f_t, f_u, g_l, g_t, g_u):
        ...
        #Case 1
        if (f_t > f_l):
            a_c = self.cubic_interp(a_l, a_t, f_l, f_t, g_l, g_t)
            a_q = self.quadratic_interp_ffg(a_l, a_t, f_l, f_t, g_l)
        ...
        #Case 2
            ...
            a_c = self.cubic_interp(a_l, a_t, f_l, f_t, g_l, g_t)   
            a_s = self.quadratic_interp_fgg(a_l, a_t, f_l, g_l, g_t)
        ...
        #Case 3
            ...
            if (self.alpha_u[self.iteration]==-1): 
                raise RuntimeError("I think this should be impossible")
        ...
        #Case 4
            ...
            a_next = self.cubic_interp(a_t, a_u, f_t, f_u, g_t, g_u) 
    ...
    def cubic_interp(self, a_1, a_2, f_1, f_2, g_1, g_2):
        a_next = -1.0
        # d = a_2 - a_1 = d(a_2)
        d = a_2 - a_1
        d2 = d*d
    ...
    def quadratic_interp_ffg(self, a_1, a_2, f_1, f_2, g_1):
        a_next = -1.0
        # d = a_2-a_1 = d(a_2)
        # Q(d) = c0 + c1*d + c2*d2
        # Q'(d) = c1 + 2*c2*d
    ...
    def quadratic_interp_fgg(self, a_1, a_2, f_1, g_1, g_2):
        a_next = -1.0
        # d = a_2-a_1 = d(a_2)
        # Q(d) = c0 + c1*d + c2*d2
        # Q'(d) = c1 + 2*c2*d
